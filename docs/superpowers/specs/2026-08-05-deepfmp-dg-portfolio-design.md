# DeepFMP-DG 求职向 GitHub 项目设计文档

> 日期：2026-08-05
> 状态：已与用户确认（用户批准：双语 README / 样例含少量真实商品图并注明仅供演示 / 命名 deepfmp-dg 并说明对 DeepFMP 的参考）

## 1. 背景与目标

基于竞赛项目 DeepFMP-DG（多模态感知视角下的商品“预期差”测度与优化研究）的真实代码、数据与实验结果，重新整理为一个**可直接发布到 GitHub 的求职向开源项目**。

- 求职方向：以 NLP 为主，同时覆盖多模态算法，目标互联网公司算法/数据岗位。
- 仓库内不出现“比赛”“论文初稿”“案例大赛”等字样，读起来是一个正经的工程+研究项目。
...
目标读者：技术面试官、招聘方工程师、同行研究者。README 需要 30 秒内讲清“这是什么、为什么有价值、结果如何”。
...
| 交付物 | 内容 | 价值 |
|---|---|---|
| 双语 README | 亮点摘要、应用场景、结果表、快速开始、架构图、Roadmap | 30 秒讲清项目 |
| 可安装包 | `pip install -e .` 后 `import deepfmp_dg` | 工程能力证明 |
| CLI | `deepfmp-dg predict ...` → JSON | 可部署性 |
| Gradio Demo | 上传卖家图/买家图/标题/评论 → 实时诊断 | 现场可玩 |
| Notebook | 样例数据全流程复现 | 完整链路展示 |
| 测试 + CI | pytest 全绿 + GitHub Actions badge | 工程素养硬证据 |
| 样例数据 + 脚本 | 开箱即用小份数据 + 全量获取说明 | 可复现性 |
| 架构/模型卡文档 | 方法、指标、局限性 | 应对技术追问 |
